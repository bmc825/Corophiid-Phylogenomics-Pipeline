...
my @protein_files = glob("$protein_dir/*_pruned.fa");
...
foreach my $protein_file (@protein_files) {
    ...
    $output_file =~ s/_pruned\.fa/.fa/;

    # Create a log file path
    my $log_file = "$output_dir/" . substr($protein_file, rindex($protein_file, '/') + 1);
    $log_file =~ s/_pruned\.fa/_log.txt/;

    # Run pal2nal.pl and redirect both stdout and stderr to the log file
    my $command = "../pal2nal.pl $protein_file $nucleotide_file -nomismatch -output fasta > $output_file 2> $log_file";
    system($command);
}
