...
This pipeline constructs an amino acid dataset from both transcriptome-derived and target-capture UCE loci. Input data from both sources are first combined into a single per-taxon directory, then reorganized into per-locus files, translated, aligned, and merged with orthogroup amino acid alignments following the same logic as the nucleotide pipeline.

#### 5.3.1 *Combine UCE nucleotide data across sources*

Create a unified directory containing all UCE nucleotide sequences (per taxon) from both transcriptome harvesting and target capture.

```bash
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_by_taxon_all

cp /bwdata3/bcummings/COROPHIOID/12-TRANSCRIPTOME_HARVESTING/04-Phyluce_explode/individual_fastas/*.fasta \
   /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_by_taxon_all/

cp /bwdata3/bcummings/COROPHIOID/13-PHYLUCE/04-UCE_extraction/individual_fastas/*.fasta \
   /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_by_taxon_all/
```

#### 5.3.2 *Prepare amino acid working directories*

```bash
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREE/01-NT_UCE_BY_LOCUS
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREE/02-AA_UCE_BY_LOCUS
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREE/03-AA_UCE_ALIGNED
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREE/04-OG_AA
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREE/05-COMBINED
```

#### 5.3.3 *Reorganize nucleotide UCEs (per taxon → per locus)*

Convert combined per-taxon FASTA files into one file per UCE locus.

```bash
perl /bwdata3/bcummings/SCRIPTS/split_uce_nt_by_locus.pl \
  --input /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_by_taxon_all \
  --output /bwdata3/bcummings/COROPHIOID/16-AATREE/01-NT_UCE_BY_LOCUS
```

#### 5.3.4 *Translate nucleotide UCE loci to amino acids*

```bash
perl /bwdata3/bcummings/SCRIPTS/translate_uce_bestframe.pl \
  --input /bwdata3/bcummings/COROPHIOID/16-AATREE/01-NT_UCE_BY_LOCUS \
  --output /bwdata3/bcummings/COROPHIOID/16-AATREE/02-AA_UCE_BY_LOCUS
```

#### 5.3.5 *Align amino acid data*

```bash
cd /bwdata3/bcummings/COROPHIOID/16-AATREE

for file in 02-AA_UCE_BY_LOCUS/*.fa; do
  base=$(basename "$file")
  mafft --auto "$file" > "03-AA_UCE_ALIGNED/$base"
done
```

#### 5.3.6 *Filter aligned amino acid loci by taxon occupancy*

```bash
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREE/03-AA_UCE_ALIGNED_min30

perl /bwdata3/bcummings/SCRIPTS/filter_loci_by_taxon_occupancy.pl \
  --input /bwdata3/bcummings/COROPHIOID/16-AATREE/03-AA_UCE_ALIGNED \
  --output /bwdata3/bcummings/COROPHIOID/16-AATREE/03-AA_UCE_ALIGNED_min30 \
  --total_taxa 68 \
  --proportion 0.3
```

#### 5.3.7 *Prepare orthogroup AA alignments*

```bash
cp /bwdata3/bcummings/COROPHIOID/07-SPECIESTREE/mintaxa75%/AA_bestgenetreemodels/input_IQTREE/*.fa \
/bwdata3/bcummings/COROPHIOID/16-AATREE/04-OG_AA


#Rename taxa to match UCE data
cd /bwdata3/bcummings/COROPHIOID/16-AATREE/04-OG_AA
...
#### 5.3.8 *Merge UCE and OG alignments*

```bash
perl /bwdata3/bcummings/SCRIPTS/filter_and_rename_uces.pl \
  --uce /bwdata3/bcummings/COROPHIOID/16-AATREE/03-AA_UCE_ALIGNED_min30 \
  --og /bwdata3/bcummings/COROPHIOID/16-AATREE/04-OG_AA \
  --mapping /bwdata3/bcummings/COROPHIOID/PROBES/uce_to_og.tsv \
  --outdir /bwdata3/bcummings/COROPHIOID/16-AATREE/05-COMBINED
```



DELETE:
#### 5.3.1 *Combine UCEs from target capture and transcriptome harvesting*

Copy per-taxon UCE FASTAs into one directory.
...
#### 5.3.2 *Clean FASTA files*

Remove gaps from UCE sequences and orthogroup sequences.

```bash
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_by_taxon_clean

for f in /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_by_taxon_all/*.fasta; do
  awk '
  /^>/ { sub(/ .*/, "", $0); print; next }
  {
    seq=toupper($0);
  ...
  }' "$f" > /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_by_taxon_clean/$(basename "$f");
done
```

```bash
mkdir -p /bwdata3/bcummings/COROPHIOID/06-PAL2NAL/mintaxa75%/01-SEQS_clean

for f in /bwdata3/bcummings/COROPHIOID/06-PAL2NAL/mintaxa75%/01-SEQS/*; do
  awk '
  /^>/ { print; next }
  {
    seq = toupper($0)
  ...
  ' "$f" > /bwdata3/bcummings/COROPHIOID/06-PAL2NAL/mintaxa75%/01-SEQS_clean/$(basename "$f")
done
```

#### 5.3.3 *Build OG amino acid reference directory*

```bash
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/OG_AA_refs

for f in /bwdata3/bcummings/COROPHIOID/06-PAL2NAL/mintaxa75%/01-SEQS_clean/*.fa*; do

  og=$(basename "$f" | sed 's/\..*//')

  awk '/^>/ {print; next}
       { gsub(/[^A-Z\*]/,"",$0); print }' "$f" \
  > /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/OG_AA_refs/${og}.faa

done

```

#### 5.3.3 *Translate UCE sequences (OG-informed frames)*

```bash
cp /bwdata3/bcummings/COROPHIOID/15-SPECIESTREE/mafft-0.3-all-taxa-combined-recovery-0.3-noTrospp-clean-completenesstargetcap-0.5_new/NUC_bestgenetreemodels_new/input_IQTREE/uce_to_og.tsv \
   /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/

mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_AA_by_taxon

perl /bwdata3/bcummings/SCRIPTS/translate_uce_to_aa_OG.pl \
  /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_by_taxon_clean \
  /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_to_og.tsv \
  /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/OG_AA_refs \
  /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_AA_by_taxon
```

Combine translated sequences by locus

```bash
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_AA_by_locus

perl /bwdata3/bcummings/SCRIPTS/combine_by_locus.pl \
  /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_AA_by_taxon \
  /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/uce_AA_by_locus
```
...
#### 5.3.4 *Amino acid alignment*

We use MAFFT directly to align sequences instead of PHYLUCE wrapper, since PHYLUCE is incompatible with amino acids.

```bash
mkdir -p /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/01-MAFFT

for f in /bwdata3/bcummings/COROPHIOID/16-AATREEPREP/01-SPLIT_BY_LOCUS/*.faa; do

    base=$(basename "$f" .faa)

    mafft --localpair --maxiterate 1000 "$f" \
    > /bwdata3/bcummings/COROPHIOID/17-AATREE/01-MAFFT/${base}.aln.faa
...
## 7. WORK COMPLETED SO FAR WITH DATES

January-December 2024: Phylotocol created for preliminary transcriptome species tree construction

January 2024: Preliminary transcriptome species tree constructed (presented at SICB 2024)

January-October 2024: Refined phylotocol method created for final transcriptome species tree construction and future target-capture species tree construction

October 2024: DNA extractions of museum specimens submitted to Arbor Biosciences for future target-capture sequencing

November 2024: Most phylogenetically informative regions identified using genesort (132 genes) and corresponding sequences submitted for bait design, synthesis, and sequencing by Arbor Biosciences

January 2025: Target capture sequences and metadata downloaded from Arbor Biosciences and uploaded to the server.

Februrary 2026: Final nucleotide species tree completed; ancestral state reconstruction completed using nucleotide species tree

April 2026: Amino acid pipeline started
...
1.1 Added target capture methods
1.2
1.3
1.4
