...
sub output_contig
  ...
  {
    my($contig, $out_contig, $rev, $begin, $end, @quals, $type) = @_;
    my($len) = scalar @quals;
    my($i, $segment);
    ...
    for ($i = 0; $i < $len; $i++)
    {
      ...
      if ($type eq 'nucleotide') {
        $segment .= "$quals[$i] "; # For nucleotides
      } elsif ($type eq 'amino_acid') {
        $segment .= "$quals[$i] "; # For amino acids
      }
    }
    print QUALOUT "$segment\n" if (length($segment));
  ...
  ###########################################################################
  # degenerate($sequence, $type) - return 1 if $sequence does not contain
  # at least one of the required bases/amino acids; else return 0.
  ###########################################################################
  ...
  sub degenerate
  {
    ...
    if ($type eq 'nucleotide') {
      ...
      return(($sequence !~ /[A-Z]/) ? 1 : 0);  # Check for valid amino acids
    }
